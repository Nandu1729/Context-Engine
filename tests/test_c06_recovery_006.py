"""Offline guards for exact four-rejection continuation, no replay or waiver."""
...
def baseline():
    return json.loads((SCRIPTS.parent / "output/c06-live-batch-177.json").read_text())


def test_four_exact_errors_and_all_672_records_preserved():
    b = baseline()
    before = deepcopy(b)
    assert len(r.checked_records(b, b["entries"])) == 672
    assert b == before
