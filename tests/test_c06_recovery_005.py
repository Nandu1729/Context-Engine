"""Offline guards for exact three-rejection continuation, no replay or waiver."""
...
def baseline():
    return json.loads((SCRIPTS.parent / "output/c06-live-batch-171.json").read_text())


def test_three_exact_errors_and_all_666_records_preserved():
    b = baseline()
    before = deepcopy(b)
    assert len(r.checked_records(b, b["entries"])) == 666
    assert b == before
