...
spec = importlib.util.spec_from_file_location(
    "c09_live", Path(__file__).resolve().parents[1] / "scripts/c09_live.py"
)
...
def experiment(tmp_path, monkeypatch, preparation):
    ...
    # Unit fixtures are explicitly TEST_ONLY on the evolving package. Actual live001
    # replay runs through the preserved0.9.1 environment, never rebased live evidence.
    monkeypatch.setattr(live, "SOURCE", preparation["runtime"]["code_hash"])
    config = {
...
def test_ground_truth_not_in_provider_request(experiment):
    manifest, _ = experiment
    _, truth, _, _ = load()
    plan = next(r for r in manifest["preparation"]["rows"] if r["case"] == "eval-middle")
    request, _, _ = live.reconstruct(plan)
    # WINDOW/900 cannot retain the middle fact. Grading must not sneak it back in.
    assert truth["eval-middle"]["answer"] not in json.dumps(request.to_wire())
