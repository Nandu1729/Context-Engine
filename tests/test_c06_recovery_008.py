"""Offline guards for exact six-rejection continuation, no replay or waiver."""
...
def baseline():
    return json.loads((SCRIPTS.parent / "output/c06-live-batch-183.json").read_text())


def test_six_exact_errors_and_all_678_records_preserved():
    b = baseline()
    before = deepcopy(b)
    assert len(r.checked_records(b, b["entries"])) == 678
    assert b == before
