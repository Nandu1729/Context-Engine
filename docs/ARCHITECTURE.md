...
4. By default,retrieve only from originals outside that planned window. The optional0.9.2 `recover_capped_window` mode also admits missing original chunks from changed WINDOW messages,excluding fully represented chunks by source-message identity. Stable chunk IDs,deterministic score tie-breaking,top-k,relevance floor and overlapping-chunk deduplication remain. See C09_REPAIR_DESIGN for the explicit PRD extension;default policy is unchanged.
