...
live = parent.live
live.RUN = ROOT / "output/private/c09-response-diagnostic-002"
live.PROTOCOL = ROOT / "docs/C09_RESPONSE_DIAGNOSTIC_PROTOCOL.md"
live.SCOPE_VERSION = "c09-response-diagnostic-002"
SHAPES = ROOT / "output/private/c09-response-shapes-002"
base_identity, base_emit = live.identity, live.emit
native_parse = transport_module.parse_completion


def identity():
    # Initial zero-call preflight is preserved, never reset or dispatched.
    old = ROOT / "output/private/c09-response-diagnostic-001"
    live.require(not list(old.glob("claim-*.json")))
...
def report(manifest, rows):
    receipts, claims = live.inspect(manifest, rows)
    new = [r for r in rows if r["id"] not in manifest["baseline"]]
    live.require(len(claims) <= 2 and len(new) <= 2)
    value = {
        "execution_id": live.fingerprint(manifest),
        "execution_identity": manifest,
        "status": "COMPLETE" if len(receipts) == 2 else "INCOMPLETE",
        "planned": 2,
        "claims": len(claims),
        "provider_attempts": len(new),
        "receipts": receipts,
        "dispositions": {"recorded": len(receipts), "missing": 2 - len(receipts)},
        "answer_quality": "DIAGNOSTIC_ONLY",
